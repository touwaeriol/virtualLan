...
namespace vl::server {
    ...
    RegisterServiceImpl::RegisterServiceImpl(std::string ipStart, std::string ipEnd, int netmask) :
            _manager(std::make_shared<EthernetAddressManager>(std::pair<std::string, std::string>(ipStart, ipEnd))),
            _netmask(netmask) {

    }


    RegisterServiceImpl::RegisterServiceImpl(std::shared_ptr<EthernetAddressManager> manager, int netmask) : _manager(
            std::move(manager)), _netmask(netmask) {
    ...
    RegisterServiceImpl::registe(::grpc::ServerContext *context, const ::vl::core::RegisterRequest *request,
        ...
        auto device = _manager->allocDevice(clientId,vl::core::split(context->peer(), ":")[1].c_str(),"default");
